                     ...
                     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  uint8_t temp = *status;
  //CODE HERE
  uint32_t index = address & L2_INDEX_MASK;
  uint32_t shiftIndex = index >> L2_INDEX_SHIFT;
  ...
  //CODE HERE
  uint32_t tag = address & L2_ADDRESS_TAG_MASK;
  uint32_t shiftedTag = tag >> L2_ADDRESS_TAG_SHIFT;
  ...
  //CODE HERE
  if((l2_cache[shiftIndex].v_d_tag >>31) == 0 || (l2_cache[shiftIndex].v_d_tag & 0x00000FFF) != shiftedTag){
    *status = temp & 0xFE; //11111110
  }
  ...
  //CODE HERE
    ...
    if((control & READ_ENABLE_MASK) != 0){
        memcpy(read_data, l2_cache[shiftIndex].cache_line, 32);
    }
    if((control & WRITE_ENABLE_MASK) != 0){
        memcpy(l2_cache[shiftIndex].cache_line, write_data, 32);
        l2_cache[shiftIndex].v_d_tag = l2_cache[shiftIndex].v_d_tag | 0x40000000;
    }
      *status = temp | 0x1;
    } 
  }
                    ...
                    uint8_t *status)
{
  ...
  //CODE HERE
  if((l2_cache[shiftIndex].v_d_tag>>31) == 0 || (l2_cache[shiftIndex].v_d_tag & 0x40000000) == 0){ //1011 0101
      memcpy(l2_cache[shiftIndex].cache_line, write_data, 32); 
      l2_cache[shiftIndex].v_d_tag = l2_cache[shiftIndex].v_d_tag | 0x80000000;
      l2_cache[shiftIndex].v_d_tag = l2_cache[shiftIndex].v_d_tag & 0xBFFFFFFF;
      l2_cache[shiftIndex].v_d_tag = l2_cache[shiftIndex].v_d_tag & 0xFFFFF000;
      l2_cache[shiftIndex].v_d_tag = l2_cache[shiftIndex].v_d_tag | shiftedTag;
  ...
  //current entry has to be written back before
  ...
  //CODE HERE
  /*
  uint32_t address, uint32_t write_data[], 
                    uint32_t *evicted_writeback_address, 
                    uint32_t evicted_writeback_data[], 
                    uint8_t *status
  */
  else{
    *evicted_writeback_address = ( (l2_cache[shiftIndex].v_d_tag & 0x00000FFF) << L2_ADDRESS_TAG_SHIFT) | (shiftIndex << L2_INDEX_SHIFT);
    memcpy(evicted_writeback_data,l2_cache[shiftIndex].cache_line,32);
    *status = temp | 0x1;
  ...
  l2_cache[shiftIndex].v_d_tag = l2_cache[shiftIndex].v_d_tag | 0x80000000;
  l2_cache[shiftIndex].v_d_tag = l2_cache[shiftIndex].v_d_tag & 0xBFFFFFFF;
  l2_cache[shiftIndex].v_d_tag = (l2_cache[shiftIndex].v_d_tag & 0xFFFFF000) | shiftedTag;
  }
