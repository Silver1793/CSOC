...

{
  ...
  if(address > main_memory_size_in_bytes){
    printf("The address is not within the size of bytes in memory");
    exit(1);
  }
  ...
  //CODE HERE
  uint32_t cache_line_address = (address & CACHE_LINE_ADDRESS_MASK)>>2;
  ...
  if((control&READ_ENABLE_MASK) == TRUE){
    int count = 0;
    uint32_t temp = cache_line_address;
    for(int i = 0 ; i < 8; i++){
      read_data[i] = main_memory[temp];
      temp++;
    }
  }
  ...
  //CODE HERE
  if((control & WRITE_ENABLE_MASK) == WRITE_ENABLE_MASK){
    int count = 0;
    uint32_t temp = cache_line_address;
    for(int i = 0; i < 8; i++){
      main_memory[temp] = write_data[i];
      temp++;
    }
  }
