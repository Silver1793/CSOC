                    ...
                    uint8_t control, uint32_t *read_data, uint8_t *status)
{
 ...
 //CODE HERE
 uint32_t index = address & L1_SET_INDEX_MASK;
 uint32_t shiftIndex = index >> L1_SET_INDEX_SHIFT;
 ...
 //CODE HERE
 uint32_t tag = address & L1_ADDRESS_TAG_MASK;
 uint32_t shiftTag = tag >> L1_ADDRESS_TAG_SHIFT;
 ...
 //CODE HERE
 uint32_t word_offset = address & WORD_OFFSET_MASK;
 uint32_t shift_word_offset = word_offset >> WORD_OFFSET_SHIFT;
 ...
 //CODE HERE
 int found = 0;
 for(int i =0; i < L1_LINES_PER_SET; i++){ //L1_NUM_CACHE_SETS
  //printf("%d",i);
   if( ( (l1_cache[shiftIndex].lines[i].v_r_d_tag >> 31) != 0) && ( ((l1_cache[shiftIndex].lines[i].v_r_d_tag & L1_ENTRY_TAG_MASK ) ) == shiftTag) ){
     *status = *status | L1_CACHE_HIT_MASK;
     found = 1;
     l1_cache[shiftIndex].lines[i].v_r_d_tag = l1_cache[shiftIndex].lines[i].v_r_d_tag | L1_RBIT_MASK;// 0x40000000;
     if((control & READ_ENABLE_MASK) != 0){
       *read_data = l1_cache[shiftIndex].lines[i].cache_line[shift_word_offset];
     }
     if((control & WRITE_ENABLE_MASK) != 0){
       l1_cache[shiftIndex].lines[i].cache_line[shift_word_offset] = write_data;
       l1_cache[shiftIndex].lines[i].v_r_d_tag = l1_cache[shiftIndex].lines[i].v_r_d_tag | L1_DIRTYBIT_MASK;// 0x20000000;
     }
     return;
   }
 }
 ...
 /*if((control & READ_ENABLE_MASK) != 0){
   memcpy(read_data, l1_cache[shiftIndex].lines[shift_word_offset].cache_line, 32);
 }
 if((control & WRITE_ENABLE_MASK) != 0){
   //strcpy(l1_cache[shiftIndex].lines[shift_word_offset].cache_line[0], write_data);
   l1_cache[shiftIndex].lines[shift_word_offset].cache_line[0] = write_data;
   l1_cache[shiftIndex].lines[shift_word_offset].v_r_d_tag = l1_cache[shiftIndex].lines[shift_word_offset].v_r_d_tag | 0x20000000;
 }*/
}
                   ...
                   uint8_t *status)
{
 ...
 //LOOP STARTS HERE
 for(int i = 0; i < L1_LINES_PER_SET; i++){ //L1_NUM_cahce_sets
 ...
 //CODE HERE
 if( (l1_cache[shiftIndex].lines[i].v_r_d_tag & L1_VBIT_MASK) == 0){
   memcpy(l1_cache[shiftIndex].lines[i].cache_line, write_data, 32);
   l1_cache[shiftIndex].lines[i].v_r_d_tag = l1_cache[shiftIndex].lines[i].v_r_d_tag | L1_VBIT_MASK; //0x80000000;
   l1_cache[shiftIndex].lines[i].v_r_d_tag = l1_cache[shiftIndex].lines[i].v_r_d_tag & ~L1_RBIT_MASK;//0x9FFFFFFF;
   l1_cache[shiftIndex].lines[i].v_r_d_tag = l1_cache[shiftIndex].lines[i].v_r_d_tag & ~L1_DIRTYBIT_MASK;
   l1_cache[shiftIndex].lines[i].v_r_d_tag = l1_cache[shiftIndex].lines[i].v_r_d_tag & ~L1_ENTRY_TAG_MASK;//0xFFFC0000;
   l1_cache[shiftIndex].lines[i].v_r_d_tag = l1_cache[shiftIndex].lines[i].v_r_d_tag | shiftTag;
   *status = *status & ~L1_CACHE_HIT_MASK; //0xFE;
   return;
 }
 //  Otherwise, we remember the first entry we encounter which has r=0 and d=0,
 //  the first entry that has r=0 and d=1, etc.
 else{
   uint32_t r = ( ((l1_cache[shiftIndex].lines[i].v_r_d_tag & L1_RBIT_MASK) >> 30) );
   uint32_t d = ( ((l1_cache[shiftIndex].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) >> 29) );
   if( ( (r == 0) && (d == 0) )/*((l1_cache[shiftIndex].lines[i].v_r_d_tag & L1_VBIT_MASK 0xE0000000) == L1_VBIT_MASK)0x80000000)*/ ){
     r0_d0_index = i;
   }
   else if( ((r == 0) && (d == 1))/*((l1_cache[shiftIndex].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK 0xE0000000) == L1_DIRTYBIT_MASK 0xA0000000)*/ ){
     r0_d1_index = i;
   }
   else if( ((r == 1) && (d == 0))/*((l1_cache[shiftIndex].lines[i].v_r_d_tag & L1_RBIT_MASK 0xE0000000) == L1_RBIT_MASK 0xC0000000)*/){ //0x1001 1111 1111
     r1_d0_index = i;
   }
 }
 //LOOP ENDS HERE
 ...
 //CODE HERE
 //printf("shiftTAG1 %d\n",l1_cache[shiftIndex].lines[2].v_r_d_tag );
 //printf("shiftTag2 %d\n", l1_cache[shiftIndex].lines[2].v_r_d_tag );
 if( ( (l1_cache[shiftIndex].lines[chosen].v_r_d_tag & L1_DIRTYBIT_MASK) >> 29) ){
   //printf("HEHEHEHHEHEEHEH");
  *evicted_writeback_address = ( (l1_cache[shiftIndex].lines[chosen].v_r_d_tag & L1_ENTRY_TAG_MASK) << L1_ADDRESS_TAG_SHIFT) | (shiftIndex << L1_SET_INDEX_SHIFT);   
  memcpy(evicted_writeback_data,l1_cache[shiftIndex].lines[chosen].cache_line,32);
    *status = *status | L1_CACHE_HIT_MASK;//0x01;
 ...
 //CODE HERE
    memcpy(l1_cache[shiftIndex].lines[chosen].cache_line,write_data,32);
 l1_cache[shiftIndex].lines[chosen].v_r_d_tag = l1_cache[shiftIndex].lines[chosen].v_r_d_tag | L1_VBIT_MASK; //0x80000000
 l1_cache[shiftIndex].lines[chosen].v_r_d_tag = l1_cache[shiftIndex].lines[chosen].v_r_d_tag & ~L1_DIRTYBIT_MASK;//0xDFFFFFFF;
 l1_cache[shiftIndex].lines[chosen].v_r_d_tag = ((l1_cache[shiftIndex].lines[chosen].v_r_d_tag & ~L1_ENTRY_TAG_MASK/*0xFFFC0000*/) | shiftTag);
}
...
void l1_clear_r_bits()
{
 ...
 for(int i = 0; i < L1_NUM_CACHE_SETS; i++){
   for(int j = 0; j < L1_LINES_PER_SET; j++){
   l1_cache[i].lines[j].v_r_d_tag = l1_cache[i].lines[j].v_r_d_tag & (~L1_RBIT_MASK); // 0xBFFFFFFF;
   }
 }
}
