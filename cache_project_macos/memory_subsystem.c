...
void memory_handle_l1_miss(uint32_t address)  
{
  ...
  //CODE HERE
  l2_cache_access(address, NULL, READ_ENABLE_MASK, cache_line, &status);
  ...
  //CODE HERE
  l1_insert_line(address, cache_line, &evicted_writeback_address, evicted_writeback_data, &status);
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  if(control == READ_ENABLE_MASK){
    main_memory_access(address, cache_line, control, cache_line);
  }
  ...
  //CODE HERE
  if(status != 0){
    main_memory_access(evicted_writeback_address, evicted_writeback_data, WRITE_ENABLE_MASK, cache_line);
  }
}
